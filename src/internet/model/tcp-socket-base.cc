/*
 * Copyrigharr (c) 2007 Georgia Tech Research Corporation
...
TcpSocketBase::GetTypeId()
{
    static TypeId tid =
        ...
        /** Exposed TARR attributes -- see tcp-socket-base.h for all attributes */

            .AddAttribute("TarrEnabled",
                            "Enable sending/processing the TARR TCP option",
                            BooleanValue(false),
                            MakeBooleanAccessor(&TcpSocketBase::m_tarrEnabled),
                            MakeBooleanChecker())
            .AddAttribute("TarrRequestRatio",
                            "Requested ACK Ratio R for TARR (0 requests = immediate ACK, max = 127)",
                            UintegerValue(2),
                            MakeUintegerAccessor(&TcpSocketBase::m_requestedAckRatio),
                            MakeUintegerChecker<uint8_t>(0, 127))
            .AddAttribute("TarrAdvertisedCapability",
                            "Announce TARR support on eligible packets",
                            BooleanValue(true),
                            MakeBooleanAccessor(&TcpSocketBase::m_localTarrCapable),
                            MakeBooleanChecker())
            .AddAttribute("TarrRequestOnData",
                            "Attach TARR request on data packets",
                            BooleanValue(true),
                            MakeBooleanAccessor(&TcpSocketBase::m_tarrRequestOnData),
                            MakeBooleanChecker())
            // Dynamic R
            .AddAttribute("TarrDynamicR",
                          "Enable dynamic R based on inter-Ack gap and sending rate (cWnd)",
                          BooleanValue(false),
                          MakeBooleanAccessor(&TcpSocketBase::m_tarrDynamicR),
                          MakeBooleanChecker());
    return tid;
}
...
TcpSocketBase::TcpSocketBase(const TcpSocketBase& sock)
    : TcpSocket(sock),
      ...
      // TARR
      m_tarrEnabled(sock.m_tarrEnabled),
      m_localTarrCapable(sock.m_localTarrCapable),
      m_requestedAckRatio(sock.m_requestedAckRatio),
      m_tarrRequestOnData(sock.m_tarrRequestOnData)
{
    NS_LOG_FUNCTION(this);
...
TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader)
{
    ...
    // TARR Delayed ACK Logic

    else
    { // In-sequence packet: ACK when threshold is reached
    uint32_t ackThreshold = m_delAckMaxCount;

    // If TARR is enabled and a valid peer ratio is active, use it instead of legacy threshold.
    if (m_tarrEnabled && m_peerTarrCapable && m_peerAckRatio > 0)
    {
    ...
    if (m_segCountForAck >= ackThreshold)
    {
        m_delAckEvent.Cancel();
        m_delAckCount = 0;
        m_segCountForAck = 0;
    ...
    // Uncomment the following else block to get old ACK behaviour
    // else
...
TcpSocketBase::AddOptions(TcpHeader& header)
{
    ...
    if (m_tarrEnabled)                         /** when TARR is enabled  */
    {
        AddOptionTARR(header);
    }
}
...
{
    Ptr<const TcpOptionTARR> tarr = DynamicCast<const TcpOptionTARR>(option);
    if (!tarr)
    {
        return;
    }

    // Valid TARR received => peer supports TARR
    m_peerTarrCapable = true;

    if (tarr->IsCapabilityAnnouncement())
    {
        //Length = 4, no ratio update
        return;
    }
    ...
    if(tarr->IsRequest())
    {
        ...
        {
            // Immediate ACK request, do not change steady-state ratio
            m_delAckEvent.Cancel();
            m_delAckCount = 0;
            m_segCountForAck = 0;
            SendEmptyPacket(TcpHeader::ACK);
        ...
        m_peerAckRatio = r;
        m_segCountForAck = 0;
        NS_LOG_INFO("Apply peer ACK ratio=" << unsigned(m_peerAckRatio));
    }
}
...
TcpSocketBase::AddOptionTARR(TcpHeader& header)
{
    ...
    // Optional: only attach on data if configured
    if (!m_tarrRequestOnData && (header.GetFlags() & TcpHeader::ACK))
    {
        return;
    }
...
/* Dynamic TARR Mechanism (Dyn-R) */
void
TcpSocketBase::UpdateTarrRatio(Time ackTime)
{
    if (!m_tarrEnabled || !m_tarrDynamicR || !m_peerTarrCapable) {
        return;
    }

    // Loss recovery and the first post-recovery segment are handled in the ACK
    // processing path upstream (R=1 on CA_LOSS entry R=2 on the first
    // segment afterwards), so this method only adapts R during steady congestion
    // avoidance. The draft recommends capping R at 2 in slow start; that is covered
    // implicitly by the acksPerRtt < 3 floor below, which lowers R whenever the
    // window is small without pinning it at 2 after a retransmission timeout

    // Measure the gap between successive ACKs
    ...
    // A gap larger than the SRTT likely spans more than one window and is discarded
    if (gap >= m_tcb->m_srtt)
    {
        return;
    }

    // Normalise by R so the spacing better reflects the reverse path
    double normGap = gap.GetSeconds() / m_requestedAckRatio;

    // Update the EWMA of the normalised gap
    m_prevNormGapEwma = m_normGapEwma;
    m_normGapEwma = (m_normGapEwma == 0.0)
                  ? normGap
                  : 0.125 * normGap + 0.875 * m_normGapEwma;

    // Warm up for 30 ACKs so the EWMA converges before any decision is taken
    ...
    // Freeze the baseline once, from the converged EWMA; all later comparisons are made relative to this value
    if (!m_baselineSet)
    {
        m_normGapBaseline = m_normGapEwma;
        m_baselineSet = true;
        m_tarrPrevCwnd = m_tcb->m_cWnd;
        return;
    }

    uint32_t mss = m_tcb->m_segmentSize;
    uint32_t cwnd = m_tcb->m_cWnd;
    uint32_t iw = m_tcb->m_initialCWnd * mss;

    // Expected number of ACKs per window under the ratio currently in effect
    double acksPerRtt = (m_requestedAckRatio > 0)
                      ? static_cast<double>(cwnd) / (m_requestedAckRatio * mss)
                      : 1.0;

    // Compares the EWMA against the frozen baseline rather than the previous sample, so
    // R reacts to a sustained drift from the learned norm instead of to per-ACK noise
    bool iatRising = (m_normGapEwma > m_normGapBaseline * 1.25);
    bool iatFalling = (m_normGapEwma < m_normGapBaseline * 0.75);
    bool cwndStable = (cwnd == m_tarrPrevCwnd);
    m_tarrPrevCwnd = cwnd;

    // Take at most one decision per 30 ACKs,
    // same variable that set the baseline is reused to pace R changes (once per 30 ACKs)
    m_tarrCaRttCount++;
    if (m_tarrCaRttCount % 30 != 0)
    {
        return;
    }

    // The branches below are evaluated in priority order; the first match wins
    if (acksPerRtt < 3.0)
    {
        // Safety floor: too few ACKs per window to sustain healthy feedback
        m_requestedAckRatio = std::max((uint8_t)1, (uint8_t)(m_requestedAckRatio - 1));
    }
    else if (iatRising && cwnd > iw)
    {
        // Increment R, sufficient feedback and healthy window
        m_requestedAckRatio = std::min((uint8_t)11, (uint8_t)(m_requestedAckRatio + 1));
    }
    else if (iatFalling && acksPerRtt > 5.0)
    {
        // Lower R
        m_requestedAckRatio = std::max((uint8_t)1, (uint8_t)(m_requestedAckRatio - 1));
    }
    else if (acksPerRtt > 10.0 && cwnd > iw && m_tcb->m_srtt.Get().GetSeconds() < 0.15)
    {
        // Bound R increments to SRTTs under 150 ms as that can be detrimental to throughput
        m_requestedAckRatio = std::min((uint8_t)11, (uint8_t)(m_requestedAckRatio + 1));
    }
    // else hold R unchanged.
